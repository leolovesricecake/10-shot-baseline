...
def predict_one(test_idx: int, retrieved_train_indices: list):
    ...
    for _ in range(NUM_SELF_CONSISTENCY):
        ...
        text_i = response.choices[0].message.content
        label = parse_prediction_to_label(text_i)
        raw_outputs.append(text_i)
        if label:
            answers.append(label)

    text = '|'.join(raw_outputs)

    if len(answers) > 0:
        pred_label = Counter(answers).most_common(1)[0][0]
    else:
        pred_label = None
