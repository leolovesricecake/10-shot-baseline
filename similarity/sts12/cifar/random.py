...
# 你的检索结果 CSV 文件路径
CSV_PATH = "/mnt/huawei/ymb/mm/baseline/dataset/CIFAR10/result/cifar10_randomn_test_to_train_top15.csv"

# 输出结果文件
OUTPUT_CSV = "qwen4b_cifar10_icl_predictions.csv"
...
def build_messages_for_one_example(test_idx: int, retrieved_train_indices: list):
    ...
    instruction = (
        ...
        f"I will give you {len(retrieved_train_indices)} labeled training examples first, "
        "and then one unlabeled test image.\n"
        "Learn from the training examples and predict the class of the final test image.\n\n"
        "Rules:\n"
        "1. Only output ONE label.\n"
        "2. Your output must be exactly one of these 10 class names.\n"
        "3. Do not explain.\n"
        "4. Do not output any extra words or punctuation.\n"
    )
    content.append({"type": "text", "text": instruction})

    # ---- ICL示例：train图 + 标签 ----
    for i, train_idx in enumerate(retrieved_train_indices, start=1):
        train_item = train_ds[int(train_idx)]
        train_img = train_item[IMG_KEY_TRAIN]
        train_label_id = int(train_item["label"])
        train_label_name = CIFAR10_LABELS[train_label_id]

        data_url = pil_to_data_url(train_img)

        content.append({
            "type": "text",
            "text": f"Example {i}: This training image has label: {train_label_name}"
        })
        content.append({
            "type": "image_url",
