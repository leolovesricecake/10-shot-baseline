...
def predict_one(test_idx: int, retrieved_train_indices: list):
    ...
    },
    {
        "role": "user",
        "content": f"""
You previously classified the image as:

{initial_text}

Check whether this classification is correct.

If it is correct, repeat the same label.
If it is incorrect, output the correct label.
...
"""
    ...
    verify_usage = getattr(verify_response, "usage", None)

    total_prompt_tokens += int(getattr(verify_usage, "prompt_tokens", 0) or 0)
    total_completion_tokens += int(getattr(verify_usage, "completion_tokens", 0) or 0)
    total_tokens += int(getattr(verify_usage, "total_tokens", 0) or 0)

    verify_text = verify_response.choices[0].message.content
    pred_label = parse_prediction_to_label(verify_text)

    combined_text = f"INITIAL: {initial_text} | VERIFIED: {verify_text}"
