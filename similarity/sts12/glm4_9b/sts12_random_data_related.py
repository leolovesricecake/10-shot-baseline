...
if __name__ == "__main__":
    for w in [10]:
        # CPU：构造 in_context（随机选择）
        in_context = "For example,"
        random_numbers = [random.randint(0, len(x_train) - 1) for _ in range(w)]
        for j in random_numbers:
            in_context += "\n" + x_train[j] + " " + "label: " + str(y_train_name[j])
        ...
        file_name = f'sts16_random_{w}_data_related.csv'
    ...
    with open(RESULT_DIR / file_name, "a") as f:
        ...
        print(f"Results for {w} saved in {file_name}\n")
