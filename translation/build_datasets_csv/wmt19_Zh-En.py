...
def main():
    ...
    output_dir = Path("/mnt/huawei/wwq/project/10-shot-baseline/datasets/wmt19_Zh-En")
    ...
    for i in range(3000):
        text = raw_train_dataset[i]['translation']['zh']
        label = raw_train_dataset[i]['translation']['en']
        train_records.append({"text": text, "label": label})

    train_df = pd.DataFrame(train_records)
    train_output_path = output_dir / "wmt19_Zh-En_train.csv"
    ...
    for i in range(test_end):
        text = raw_test_dataset[i]['translation']['zh']
        label = raw_test_dataset[i]['translation']['en']
        test_records.append({"text": text, "label": label})

    test_df = pd.DataFrame(test_records)
    test_output_path = output_dir / "wmt19_Zh-En_test.csv"
